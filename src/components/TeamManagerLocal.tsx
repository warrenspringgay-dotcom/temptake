...
function downloadICS(events: Array<{ title: string; date: string }>, filename = "training-reminders.ics") {
...
export default function TeamManagerLocal() {
  ...
  const allTrainings = useMemo(() => staff.flatMap((s) => s.trainings.map((t) => ({ s, t }))), [staff]);
  const stats = useMemo(() => {
  ...
  const [staffModal, setStaffModal] = useState<{ open: boolean; edit?: Staff | null }>({ open: false });
  const [trainingModal, setTrainingModal] = useState<{ open: boolean; staffId?: string; edit?: Training | null }>({ open: false });

  function saveStaff(draft: Omit<Staff, "id" | "trainings"> & { id?: string; trainings?: Training[] }) {
    setStaff((prev) => {
  ...
  function removeTraining(staffId: string, trainingId: string) {
    setStaff((prev) => prev.map((s) => (s.id === staffId ? { ...s, trainings: s.trainings.filter((t) => t.id !== trainingId) } : s)));
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-6xl p-4 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">Add staff so they can log temperatures (the logger checks initials against this list).</p>
          </div>
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => {
                ...
                if (!events.length) { alert("No upcoming/overdue expiries."); return; }
                    ...
                    <details>
                      <summary className="flex items-center justify-between cursor-pointer">
                        <div className="min-w-0">
                          <div className="font-medium">
                            ...
                            {s.jobTitle ? <span className="text-slate-500 font-normal"> — {s.jobTitle}</span> : null}
                          </div>
                      ...
                      <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div className="text-right">
                          <Button variant="outline" onClick={() => setTrainingModal({ open: true, staffId: s.id })}>+ Add training</Button>
                          <Button variant="outline" className="ml-2" onClick={() => setStaffModal({ open: true, edit: s })}>Edit</Button>
                          <button className="ml-3 text-red-600 text-sm hover:underline" onClick={() => { if (confirm("Delete this staff member?")) removeStaff(s.id); }}>
                            Delete
                          </button>
                        </div>
                      </div>
                                ...
                                return (
                                  <tr key={t.id} className="border-t border-gray-200">
                                    ...
                                    <td className="py-2 pr-3 text-right">
                                      <button className="text-slate-700 hover:underline mr-3" onClick={() => setTrainingModal({ open: true, staffId: s.id, edit: t })}>
                                        Edit
                                      </button>
                                      <button className="text-red-600 hover:underline" onClick={() => removeTraining(s.id, t.id)}>
                                        Delete
                                      </button>
                                    </td>
                                  </tr>
      ...
      {staffModal.open && (
        <StaffModal initial={staffModal.edit ?? null} onClose={() => setStaffModal({ open: false })} onSave={saveStaff} allStaff={staff} />
      )}
      {trainingModal.open && trainingModal.staffId && (
        <TrainingModal
          ...
          onSave={(t) => saveTraining(trainingModal.staffId!, t)}
        />
      )}
    </div>
  );
...
function StaffModal({
  ...
  function submit() {
    ...
    const clash = allStaff.find((s) => s.initials.toUpperCase() === form.initials.toUpperCase() && s.id !== form.id);
    if (clash) {
      ...
      <div className="w-full max-w-lg rounded-xl border border-gray-200 bg-white shadow-lg">
        ...
        <div className="px-6 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.initials} onChange={(e) => setForm({ ...form, initials: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm mb-1">Name *</label>
            <input className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </div>
          <div>
            <label className="block text-sm mb-1">Job title</label>
            <input className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })} />
          </div>
          <div>
      ...
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white shadow-lg">
        ...
        <div className="px-6 py-4 grid grid-cols-1 gap-3">
          <div>
            ...
            <input className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} />
          </div>
          <div>
