...
type RoutineLite = { id: string; name: string; items_count: number };
...
export default function FoodTempLogger({ initials: initialsSeed, locations: locationsSeed }: Props) {
  ...
  async function openRoutinePicker() {
    ...
    try {
      ...
      const q = supabase
        .from("routines")
        .select("id,name,organisation_id,org_id")
        .order("name", { ascending: true });
      const { data, error } = orgId
        ? await q.or(`organisation_id.eq.${orgId},org_id.eq.${orgId}`)
        : await q;
      ...
      if (ids.length) {
        const { data: items } = await supabase
          ...
          .select("routine_id")
          .in("routine_id", ids);
        for (const r of ids) counts[r] = 0;
        for (const it of items ?? []) counts[it.routine_id] = (counts[it.routine_id] ?? 0) + 1;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {routineOpen && (
        <div className="fixed inset-0 z-50 bg-black/30" onClick={() => setRoutineOpen(false)}>
          <div
            className="mx-auto mt-8 w-[min(640px,92vw)] overflow-hidden rounded-2xl border bg-white shadow"
            onClick={(e) => e.stopPropagation()}
            role="dialog"
            aria-modal="true"
          >
            <div className="flex items-center justify-between border-b px-4 py-3">
              <div className="text-base font-semibold">Pick a routine</div>
              <button className="rounded-md p-2 hover:bg-gray-100" onClick={() => setRoutineOpen(false)} aria-label="Close">
                Close
              </button>
            </div>
            ...
            <div className="max-h-[60vh] overflow-auto p-3">
              {routineLoading ? (
                ...
                <div className="p-4 text-center text-sm text-gray-500">No routines yet.</div>
              ) : (
                  ...
                  {routines.map((r) => (
                    <li key={r.id}>
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-500">{r.items_count} entries</span>
                      </Link>
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex items-center justify-end gap-2 border-t px-3 py-2">
              ...
              <button className="rounded-md px-3 py-1.5 text-sm hover:bg-gray-50" onClick={() => setRoutineOpen(false)}>
                Cancel
              </button>
            </div>
          </div>
