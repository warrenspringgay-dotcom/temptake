...
export default function TeamManager() {
  ...
  // View-member modal (with training + QC)
  ...
  // QC reviews (manager quality checks)
  ...
  const initialsOptions = useMemo(() => {
    return Array.from(
      new Set(
        rows
          .map((r) => r.initials?.trim() || deriveInitials(r.name))
          .filter(Boolean)
  ...
  async function openView(r: TeamMember) {
    ...
    const qc = await listStaffQcReviewsForStaff(staffId);
    ...
    // force manager selection (because manager_id must be a staff row)
    setQcForm({
      managerInitials: "",
      reviewed_on: new Date().toISOString().slice(0, 10),
      score: 3,
  ...
  async function addQcReview() {
    if (!viewStaffId) return;
    if (!qcForm.managerInitials.trim()) {
      alert("Select manager initials.");
      return;
    }
    setQcSaving(true);
    try {
      await insertStaffQcReview(viewStaffId, {
        manager_initials: qcForm.managerInitials.trim().toUpperCase(),
        reviewed_on: qcForm.reviewed_on,
        score: qcForm.score,
        notes: qcForm.notes?.trim() || null,
      });
      setQcReviews(await listStaffQcReviewsForStaff(viewStaffId));
      setQcForm((f) => ({
        ...f,
        reviewed_on: new Date().toISOString().slice(0, 10),
        score: 3,
      ...
      alert(`Failed to save QC review: ${e?.message}`);
    } finally {
      setQcSaving(false);
    }
  }

  async function removeQcReview(id: string) {
    if (!confirm("Delete this QC review?")) return;
    try {
      await deleteStaffQcReview(id);
      if (viewStaffId) setQcReviews(await listStaffQcReviewsForStaff(viewStaffId));
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      {/* View Member + Training + QC Modal (full-screen on mobile) */}
      {viewOpen && viewMember && (
        <div className="fixed inset-0 z-50 bg-black/40" onClick={() => setViewOpen(false)}>
          ...
          >
            ...
            {/* Add training inline */}
            <div className="mx-4 mt-4 rounded-2xl border bg-white p-3 sm:mx-0">
            ...
            <div className="mx-4 mt-4 rounded-2xl border bg-white p-3 sm:mx-0">
              <div className="mb-2 flex items-center justify-between">
                <h4 className="text-sm font-semibold">Manager QC</h4>
              </div>
                    ...
                    <tbody>
                      {qcReviews.map((q) => (
                        <tr key={q.id} className="border-t align-top">
                          <td className="py-2 pr-3 whitespace-nowrap">{q.reviewed_on}</td>
                          ...
                          <td className="py-2 pr-3 whitespace-nowrap">
                            {q.manager_initials
                              ? `${q.manager_initials}${q.manager_name ? ` · ${q.manager_name}` : ""}`
                              : "—"}
                          </td>
                          <td className="py-2 pr-3 min-w-[220px]">{q.notes || "—"}</td>
                          <td className="py-2 pr-3 text-right">
                            <button
                              onClick={() => removeQcReview(q.id)}
                              className="h-8 rounded-xl border px-3 text-xs text-red-700 hover:bg-gray-50"
                              title="Delete"
                            >
                              Delete
                            </button>
            ...
            {/* Add QC review */}
            <div className="mx-4 mt-4 rounded-2xl border bg-white p-3 sm:mx-0">
              <h4 className="mb-2 text-sm font-semibold">Add QC review</h4>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                <div className="sm:col-span-1">
                  <label className="mb-1 block text-xs text-gray-500">Manager initials</label>
                  <select
                    className="w-full rounded-xl border px-3 py-2 uppercase"
                    value={qcForm.managerInitials}
                    onChange={(e) =>
                      setQcForm((f) => ({ ...f, managerInitials: e.target.value.toUpperCase() }))
                    }
                  >
                    <option value="">Select…</option>
                    {initialsOptions.map((ini) => (
                  ...
                  <div className="mt-1 text-[11px] text-gray-500">
                    This sets <code className="rounded bg-gray-100 px-1">manager_id</code>.
                  </div>
                </div>

                <div>
                  <label className="mb-1 block text-xs text-gray-500">Reviewed on</label>
                  <input
                    type="date"
                    className="w-full rounded-xl border px-3 py-2"
                ...
                <div className="sm:col-span-4">
                  ...
                  <textarea
                    ...
                    placeholder="What went well / what needs improving…"
                  />
                </div>
              </div>
              ...
              <div className="mt-3 flex justify-end">
                <button
                  ...
                  className={cls(
                    "rounded-xl px-4 py-2 text-sm font-medium text-white",
                    qcSaving ? "bg-gray-400" : "bg-black hover:bg-gray-900"
                  )}
                >
                  {qcSaving ? "Saving…" : "Save QC review"}
                </button>
              </div>
            </div>
