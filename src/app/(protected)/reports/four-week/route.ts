...
import { getFourWeeklyReview, fourWeekSummaryToLines } from '@/app/actions/reports';

/**
 * Minimal PDF generator (single page, Helvetica).
 * No dependencies. No drama. Just bytes.
 */
...
function buildSimplePdf(lines: string[]) {
  const fontSize = 11;
  const left = 44; // margin
  const top = 800; // start y
  const leading = 14; // line height

  const maxLines = 52; // keep on one page (enough for our short report)
...
export async function GET(req: NextRequest) {
  // Optional scoping:
  // /api/reports/four-week?locationId=xxxx&to=yyyy-mm-dd
  const { searchParams } = new URL(req.url);
  const locationId = searchParams.get('locationId') ?? undefined;
  const to = searchParams.get('to') ?? undefined;

  const summary = await getFourWeeklyReview({ locationId, to });
