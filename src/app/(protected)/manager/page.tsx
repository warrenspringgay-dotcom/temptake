...
export default function ManagerDashboardPage() {
  ...
  async function refreshAll() {
    ...
    try {
      ...
      const [
        ...
        incidents7dRes,
        trainingsRes,
        teamMembersForTrainingRes, // ✅ ADDED
        signoffsDayRes,
        allergenLogsRes,
      ] = await Promise.all([
        supabase
        ...
        // ✅ include team_member_id so we can map to a location
        supabase
          .from("trainings")
          .select("id, expires_on, team_member_id")
          .eq("org_id", orgId)
          .limit(5000),

        // ✅ NEW: map team member -> location_id for filtering
        supabase
          .from("team_members")
      ...
      const firstErr =
        ...
        trainingsRes.error ||
        teamMembersForTrainingRes.error || // ✅ ADDED
      ...
      // ✅ TRAINING KPI NOW RESPECTS SELECTED LOCATION
      const memberLocRows: Array<{ id: string; location_id: string | null }> =
        ((teamMembersForTrainingRes.data as any[]) ?? []).map((r) => ({
          id: String(r.id),
          location_id: r.location_id ? String(r.location_id) : null,
        }));

      const memberLocMap = new Map<string, string | null>();
      for (const r of memberLocRows) memberLocMap.set(String(r.id), r.location_id);

      const tRows: Array<{ expires_on: string | null; team_member_id: string | null }> =
        ((trainingsRes.data as any[]) ?? []).map((r) => ({
          expires_on: r.expires_on ? String(r.expires_on) : null,
          team_member_id: r.team_member_id ? String(r.team_member_id) : null,
        }));

      // keep only trainings for members at this location
      const tRowsForLocation = tRows.filter((t) => {
        if (!t.team_member_id) return false; // orphan training, ignore
        const loc = memberLocMap.get(String(t.team_member_id)) ?? null;
        return loc === String(locationId);
      });

      let expired = 0;
      let dueSoon = 0;
      for (const t of tRowsForLocation) {
  ...
  return (
    <>
      ...
      {/* the rest of your component remains exactly as you pasted it */}
      {/* ... */}
