...
/* Keys if you’re loading from LocalStorage (keep if you use them elsewhere) */
...
function lastMonthsRange(n: number) {
  ...
  const from = new Date(Date.UTC(start.getFullYear(), start.getMonth(), 1))
    .toISOString()
    .slice(0, 10);
...
/* ---------------- jsPDF + AutoTable type-safe dynamic import ------------- */

/** Minimal options we actually use when calling autoTable */
...
/** Extend jsPDF with the piece autoTable adds (lastAutoTable.finalY) */
...
async function getPdfBits() {
  ...
  // the plugin exports default in ESM; fallback for CJS shapes
  const autoTable: (doc: JsPDFWithAutoTable, opts: AutoTableOptions) => void =
    ((mod as any).default ?? (mod as any).autoTable) as (
...
export default function ReportsPageClient() {
  ...
  // If you still use LocalStorage as your backing store:
  ...
  async function exportPdf(rangeLabel: string) {
    ...
    // create the doc and assert it supports lastAutoTable
  ...
  function quickReport() {
    ...
    setFrom(r.start);
    setTo(r.end);
    void exportPdf("Quick report (last 3 months)");
  }
